...
// Client component for marketplace
function MarketplacePage() {
  // Pre-calculate the floor values for ratings to avoid hydration mismatch
  const agentsWithStaticRatings = sampleAgents.map(agent => ({
    ...agent,
    flooredRating: Math.floor(agent.rating || 0)
  }));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {agentsWithStaticRatings.map((agent) => (
                  ...
                  <div className="flex items-center">
                    {[0, 1, 2, 3, 4].map((rating) => (
                      <svg
                        key={rating}
                        className={`h-5 w-5 ${
                          rating < agent.flooredRating ? 'text-yellow-400' : 'text-gray-300'
                        }`}
